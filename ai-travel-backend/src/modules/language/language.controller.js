...
class LanguageController {
  ...
  async getById(req, res, next) { try { const item = await service.getById(req.params.id); if (!item) return res.status(NOT_FOUND).json({ message: 'Language not found' }); res.status(OK).json(item); } catch (e) { next(e); } }
  async update(req, res, next) { try { res.status(OK).json(await service.update(req.params.id, req.body)); } catch (e) { next(e); } }
  async delete(req, res, next) { try { await service.delete(req.params.id); res.status(OK).json({ message: 'Language deleted' }); } catch (e) { next(e); } }
}
